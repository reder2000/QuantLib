...
namespace QuantLib {
    ...
    */
    template <class Date>
    class Botswana : public Calendar<Date> {
    private:
    ...
    public:
      Botswana() {
        ...
        static std::shared_ptr<Calendar<Date>::Impl> impl(new Botswana::Impl);
        impl_ = impl;
    }
    };

    template <class Date> inline
        bool Botswana<Date>::Impl::isBusinessDay(const Date& date) const {
