...
namespace QuantLib {
    ...
    */
    template <class Date>
    class Japan : public Calendar<Date> {
      private:
        class Impl : public Calendar<Date>::Impl {
          public:
            std::string name() const { return "Japan"; }
            bool isWeekend(Weekday) const { return w == Saturday || w == Sunday; }
     ...
     template <class Date>
    inline bool Japan<Date>::Impl::isBusinessDay(const Date& date) const {
        ...
        Day ve = // vernal equinox day
            Day(exact_vernal_equinox_time + moving_amount - number_of_leap_years);
        Day ae = // autumnal equinox day
            Day(exact_autumnal_equinox_time + moving_amount - number_of_leap_years);
        // checks
        if (isWeekend(w)
            // New Year's Day
            || (d == 1 && m == January)
